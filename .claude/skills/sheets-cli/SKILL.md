...
> **Installation:** `sheets-cli` may not be available in PATH. If the binary is not found, use the **googleapis fallback** below.

## googleapis Fallback (when sheets-cli binary unavailable)

Auth credentials are stored at `~/.sheets-cli/`:
- `credentials.json` — OAuth client credentials (Desktop app)
- `token.json` — Access + refresh token (scopes: `spreadsheets` + `drive.readonly`)
...
const creds = JSON.parse(fs.readFileSync(path.join(AUTH_DIR, 'credentials.json'), 'utf8'));
const token = JSON.parse(fs.readFileSync(path.join(AUTH_DIR, 'token.json'), 'utf8'));
...
auth.on('tokens', (t) => {
  fs.writeFileSync(path.join(AUTH_DIR, 'token.json'),
    JSON.stringify({ ...token, ...t }, null, 2));
});

const sheets = google.sheets({ version: 'v4', auth });

// Read: sheets.spreadsheets.values.get({ spreadsheetId, range })
// Write: sheets.spreadsheets.values.batchUpdate({ spreadsheetId, requestBody: { valueInputOption: 'USER_ENTERED', data: [{range, values}] } })
```

### Reference Script

See `scripts/update-display-text.js` for a complete example (reads Abilities tab, batch-updates Display Text column).
